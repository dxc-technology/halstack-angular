...
export class DxcCrudTableComponent implements OnInit, ControlValueAccessor, OnChanges, AfterViewInit, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes.editableColumns && changes.editableColumns.currentValue && changes.editableColumns.currentValue.filter(({ fieldType }) => fieldType === EFieldsType.dropdown).size  > 0) {
